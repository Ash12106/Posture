...
export default function ThreeDView({ poseData, rulaScore, rebaScore }: ThreeDViewProps) {
  ...
  return (
    <div className="bg-gradient-to-br from-dark-card to-dark-secondary rounded-xl shadow-2xl border border-gray-700 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="bg-gradient-to-r from-blue-500 to-purple-500 p-2 rounded-lg">
              <span className="material-icon text-white text-xl">view_in_ar</span>
            </div>
            <div>
              <h3 className="text-xl font-bold text-white">3D Skeleton View</h3>
              <p className="text-xs text-gray-300">RULA-colored pose visualization</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <div className="flex items-center space-x-2">
              <div className={`w-3 h-3 rounded-full ${poseData ? 'bg-green-400 animate-pulse shadow-lg shadow-green-400/50' : 'bg-gray-500'}`}></div>
              <div className="text-right">
                <span className="text-sm font-medium text-white">
                  {poseData ? 'Rendering' : 'Standby'}
                </span>
                <p className="text-xs text-gray-300">
                  {poseData ? '3D skeleton' : 'No pose data'}
                </p>
              </div>
            </div>
            <button
              onClick={resetView}
              className="p-2.5 hover:bg-gray-600/50 rounded-lg transition-all duration-200 text-white hover:scale-105 bg-gray-700/50"
              title="Reset View"
            >
      ...
      <div className="relative">
        <div
          ref={containerRef}
          className="w-full h-96 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 relative"
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center space-y-4">
                <div className="bg-gray-700/50 p-6 rounded-full mb-4">
                  <span className="material-icon text-6xl text-gray-400">accessibility_new</span>
                </div>
                <div>
                  <h4 className="text-lg font-semibold text-gray-300 mb-2">3D Skeleton Inactive</h4>
                  <p className="text-sm text-gray-500 mb-4">Start pose detection to view 3D visualization</p>
                </div>
                <div className="space-y-2 text-xs text-gray-600">
                  <div className="flex items-center justify-center space-x-2">
                    <span className="material-icon text-sm">mouse</span>
                    <span>Drag to rotate • Scroll to zoom</span>
                  </div>
                  <div className="flex items-center justify-center space-x-2">
                    <span className="material-icon text-sm">palette</span>
                    <span>Colors indicate RULA risk levels</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Compact Assessment Score Overlay */}
        {(rulaScore || rebaScore) && (
          <div className="absolute bottom-4 left-4 bg-gradient-to-br from-gray-800/95 to-gray-900/95 backdrop-blur-sm border border-gray-600/50 rounded-lg p-2 text-xs shadow-xl">
            <div className="flex items-center space-x-1 mb-2">
              <div className="bg-orange-500/20 p-1 rounded">
                <span className="material-icon text-orange-400 text-xs">assessment</span>
              </div>
            ...
            <div className="space-y-1">
              {rulaScore && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">RULA:</span>
                  <div className="flex items-center space-x-1">
                    <div className={`px-1.5 py-0.5 rounded text-xs font-bold ${
                      rulaScore.finalScore <= 2 ? 'bg-green-500/20 text-green-400' :
                      rulaScore.finalScore <= 4 ? 'bg-yellow-500/20 text-yellow-400' :
                      rulaScore.finalScore <= 6 ? 'bg-orange-500/20 text-orange-400' : 'bg-red-500/20 text-red-400'
                    }`}>
                      {String(rulaScore.finalScore || 'N/A')}
                    </div>
                  </div>
                </div>
              )}
              ...
              {rebaScore && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">REBA:</span>
                  <div className="flex items-center space-x-1">
                    <div className={`px-1.5 py-0.5 rounded text-xs font-bold ${
                      rebaScore.finalScore === 1 ? 'bg-green-500/20 text-green-400' :
                      rebaScore.finalScore <= 3 ? 'bg-yellow-500/20 text-yellow-400' :
                      rebaScore.finalScore <= 7 ? 'bg-orange-500/20 text-orange-400' :
                      rebaScore.finalScore <= 10 ? 'bg-red-500/20 text-red-400' : 'bg-red-500/30 text-red-300'
                    }`}>
                      {String(rebaScore.finalScore || 'N/A')}
                    </div>
                  </div>
                </div>
              )}
        ...
        {poseData && (rulaScore || rebaScore) && (
          <div className="absolute bottom-4 right-4 bg-gradient-to-br from-gray-800/95 to-gray-900/95 backdrop-blur-sm border border-gray-600/50 rounded-lg p-2 text-xs shadow-xl">
            <div className="flex items-center space-x-1 mb-2">
              <div className="bg-blue-500/20 p-1 rounded">
                <span className="material-icon text-blue-400 text-xs">bug_report</span>
              </div>
              <span className="font-medium text-white text-xs">Debug</span>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-white font-mono text-xs">
                  {poseData.keypoints?.filter((kp: any) => kp && kp.score > 0.3).length || 0}/{poseData.keypoints?.length || 0}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className={`font-mono text-xs ${
                  (poseData.keypoints?.filter((kp: any) => kp && kp.score > 0.3).length || 0) >= 15 ? 'text-green-400' : 
                  (poseData.keypoints?.filter((kp: any) => kp && kp.score > 0.3).length || 0) >= 10 ? 'text-yellow-400' : 'text-red-400'
                }`}>
                  {Math.round(((poseData.keypoints?.filter((kp: any) => kp && kp.score > 0.3).length || 0) / 17) * 100)}%
                </span>
              </div>
              ...
              {rebaScore && (
                <div className="flex justify-between items-center">
                  <span className="text-gray-400">REBA:</span>
                  <span className={`font-mono text-xs font-bold ${
                    rebaScore.finalScore === 1 ? 'text-green-400' :
                    rebaScore.finalScore <= 3 ? 'text-yellow-400' :
                    rebaScore.finalScore <= 7 ? 'text-orange-400' :
                    rebaScore.finalScore <= 10 ? 'text-red-400' : 'text-red-300'
                  }`}>{rebaScore.finalScore || 'N/A'}</span>
                </div>
              )}
            </div>
          </div>
