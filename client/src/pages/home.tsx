...
import RebaRecordingPanel from "@/components/pose-detection/reba-recording-panel";
import ReportGeneratorComponent from "@/components/pose-detection/report-generator";
import RecommendationPanel from "@/components/pose-detection/recommendation-panel";
...
export default function Home() {
  ...
  const [assessmentType, setAssessmentType] = useState<'rula' | 'reba' | 'both'>('both');
  const [loadingMessage, setLoadingMessage] = useState("Initializing ErgoTrack...");
  ...
  const [enabledCategories, setEnabledCategories] = useState(['posture', 'movement', 'breaks', 'environment']);
  ...
  const {
    ...
    isProcessing,
    initializeModel
  } = usePoseDetection(videoRef, canvasRef, cameraActive, assessmentType);
      ...
      <main className="container mx-auto px-6 py-6">
        ...
        <div className="mb-6">
          <div className="bg-dark-card rounded-lg p-4 shadow-lg">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-4">
                <span className="text-sm text-text-secondary">Assessment Type:</span>
                <div className="flex items-center space-x-2 bg-dark-secondary rounded-lg p-1">
                  <button
                    onClick={() => setAssessmentType('rula')}
                  ...
                  <button
                    onClick={() => setAssessmentType('reba')}
                    className={`px-3 py-1 rounded text-sm transition-colors ${
                      assessmentType === 'reba' 
                        ? 'bg-material-blue text-white' 
                        : 'text-text-secondary hover:text-white'
                  ...
                  <button
                    onClick={() => setAssessmentType('both')}
                    className={`px-3 py-1 rounded text-sm transition-colors ${
                      assessmentType === 'both' 
                        ? 'bg-material-blue text-white' 
                        : 'text-text-secondary hover:text-white'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <div data-component="three-d-view">
            <ThreeDView 
              poseData={poseData} 
              rulaScore={rulaScore} 
              rebaScore={rebaScore}
          ...
          <div data-component="camera-view">
            <CameraView 
              videoRef={videoRef} 
              canvasRef={canvasRef} 
        ...
        <div className={`grid gap-6 mb-6 ${
          assessmentType === 'both' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'
        }`} data-component="assessment-view">
          {(assessmentType === 'rula' || assessmentType === 'both') && (
            <RulaAssessment 
              rulaScore={rulaScore} 
              poseData={poseData} 
          ...
          {(assessmentType === 'reba' || assessmentType === 'both') && (
            <RebaAssessment 
              rebaScore={rebaScore} 
              poseData={poseData} 
              isProcessing={isProcessing}
        ...
        {/* Recording and Analysis Panels */}
        <div className={`grid gap-6 mb-6 ${
          assessmentType === 'both' ? 'grid-cols-1 xl:grid-cols-2' : 'grid-cols-1'
        }`}>
          {/* RULA Recording Panel */}
          {(assessmentType === 'rula' || assessmentType === 'both') && (
            <RecordingPanel
        ...
        {/* Smart Recommendations and Report Generator */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
          <RecommendationPanel
            ...
            onVisibilityChange={setRecommendationsVisible}
